...
export function validateDisplayForm(values: DisplayFormValues): readonly string[] {
  if (values.displayMode === "physical") return [];
  const errors: string[] = [];
  if (!Number.isInteger(values.width) || values.width < 640 || values.width > 3840) errors.push("Width must be 640–3840 pixels.");
  if (!Number.isInteger(values.height) || values.height < 480 || values.height > 2160) errors.push("Height must be 480–2160 pixels.");
  if (!Number.isInteger(values.dpi) || values.dpi < 120 || values.dpi > 640) errors.push("DPI must be 120–640.");
  if (!Number.isFinite(values.videoBitRateMbps) || values.videoBitRateMbps < 2 || values.videoBitRateMbps > 50) errors.push("Bitrate must be 2–50 Mbps.");
  if (!Number.isInteger(values.maxFps) || values.maxFps < 15 || values.maxFps > 120) errors.push("Frame rate must be 15–120 fps.");
  if (values.startApp && !PACKAGE_PATTERN.test(values.startApp)) errors.push("Application must be an exact Android package name.");
  if (!values.startApp && !values.systemDecorations) errors.push("Select an application when system decorations are disabled.");
  return errors;
}

export function buildSessionRequest(values: DisplayFormValues, serial: string): Record<string, unknown> {
...
export function alignedFlexSize(
