...
export class ScrcpyV41Adapter {
    ...
    validateServer(serverInfo) {
        if (serverInfo.scrcpyVersion !== SCRCPY_VERSION) return { compatible: false, reason: `expected scrcpy ${SCRCPY_VERSION}, got ${serverInfo.scrcpyVersion}` };
        if (serverInfo.adapterId !== ADAPTER_ID) return { compatible: false, reason: `expected adapter ${ADAPTER_ID}, got ${serverInfo.adapterId}` };
        return { compatible: true };
    }
    async connect(transport, options) {
        if (!options.video && !options.audio && !options.control) throw new InvalidProtocolValueError("at least one scrcpy channel must be enabled");
        ...
        const firstReader = videoStream ? new AsyncByteReader(videoStream, { label: "scrcpy video" }) : audioStream ? new AsyncByteReader(audioStream, { label: "scrcpy audio" }) : controlPair ? new AsyncByteReader(controlPair.readable, { label: "scrcpy control" }) : null;
        if (!firstReader) throw new VersionMismatchError("unable to select first scrcpy channel");
        if (options.expectDummyByte ?? true) await readDummyByte(firstReader);
        ...
        const audioReader = audioStream ? (videoStream ? new AsyncByteReader(audioStream, { label: "scrcpy audio" }) : firstReader) : null;
        const controlReader = controlPair ? (videoStream || audioStream ? new AsyncByteReader(controlPair.readable, { label: "scrcpy control" }) : firstReader) : null;
        const videoParser = videoReader ? new ScrcpyMediaStreamParser(videoReader, { kind: "video", expectStreamMeta: options.expectStreamMeta ?? true }) : null;
        let audioParser = audioReader ? new ScrcpyMediaStreamParser(audioReader, { kind: "audio", expectStreamMeta: options.expectStreamMeta ?? true }) : null;
        ...
        if (audioParser) {
            try { audioHeader = await audioParser.readHeader(); }
            catch (error) { if (error instanceof StreamDisabledError) audioParser = null; else throw error; }
        }
        return {
            device, videoHeader, audioHeader,
            readVideoPacket: async () => { if (!videoParser) throw new VersionMismatchError("video channel is disabled"); return videoParser.readPacket(); },
            readAudioPacket: async () => { if (!audioParser) throw new VersionMismatchError("audio channel is disabled"); return audioParser.readPacket(); },
            readDeviceMessage: async () => { if (!deviceParser) throw new VersionMismatchError("control channel is disabled"); return deviceParser.read(); },
            sendControl: async (message) => { if (!controlScheduler) throw new VersionMismatchError("control channel is disabled"); await controlScheduler.send(message); },
            close: async () => { try { await controlScheduler?.close(); await controlWriter?.close(); } finally { await transport.close(); } },
        };
    }
}
