...
import { CODEC_NAMES, CodecId, PACKET_FLAG_CONFIG, PACKET_FLAG_KEY_FRAME, PACKET_FLAG_SESSION, PACKET_HEADER_LENGTH, PACKET_PTS_MASK, } from "./constants.js";
export class ScrcpyMediaStreamParser {
    ...
    async readPacket() {
        ...
        if (packetLength > this.#maximumPacketSize) {
            throw new InvalidProtocolValueError(`scrcpy media packet length ${packetLength} exceeds limit ${this.#maximumPacketSize}`);
        }
...
function parseVideoSession(raw) {
    const session = {
