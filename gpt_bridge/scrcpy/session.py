...
from gpt_bridge.adb.client import AdbBackend, SpawnedAdbProcess
from gpt_bridge.errors import (
    DeviceNotFoundError,
    DeviceNotReadyError,
...
class SessionManager:
    ...
    async def start_monitor(self) -> None:
        if self._monitor_task and not self._monitor_task.done():
            return
        self._closed = False
        self._monitor_task = asyncio.create_task(self._monitor_devices(), name="gpt-bridge-device-monitor")
    ...
    async def _start_session_resources(
        ...
        session.process_watch_task = asyncio.create_task(
            ...
            name=f"gpt-bridge-server-watch-{session.session_id}",
        )
