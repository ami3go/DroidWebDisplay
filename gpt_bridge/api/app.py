...
from gpt_bridge.errors import BridgeError
...
from gpt_bridge.transfers import AdbSyncClient, AutoDownloadConfig, AutoDownloadMonitor, DuplicatePolicy, TransferManager
from gpt_bridge.transfers.paths import storage_root_list
from gpt_bridge.websocket import relay_control_websocket, relay_media_websocket
...
def create_app(
    *,
    ...
    app = FastAPI(
        title="Gpt-Bridge Local Service",
        version=__version__,
    ...
    async def transfer_upload(
        ...
        serial: str = Form(...),
        destination_path: str = Form(default="/sdcard/Download/GptBridgeInbox", alias="destinationPath"),
    ...
    def network_config_from_request(body: NetworkConfigRequest, *, generate: bool) -> tuple[NetworkAccessConfig, dict[str, Any] | None]:
        ...
        if body.certificate_source == "generated":
            certificate_path = resolved_config.resolved_transfer_data_directory / "tls" / "gpt-bridge-cert.pem"
            private_key_path = resolved_config.resolved_transfer_data_directory / "tls" / "gpt-bridge-key.pem"
    ...
    async def network_public_certificate() -> FileResponse:
        ...
        return FileResponse(path, media_type="application/x-pem-file", filename="gpt-bridge-cert.pem")
