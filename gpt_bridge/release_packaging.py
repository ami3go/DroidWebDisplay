...
setlocal
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0GptBridge.ps1" %*
exit /b %ERRORLEVEL%
'''
...
'''
    if target == "windows":
        return {"GptBridge.ps1": windows_ps1, "GptBridge.cmd": windows_cmd}
    if target == "linux":
        return {"GptBridge.sh": linux_sh}
    return {"GptBridge.ps1": windows_ps1, "GptBridge.cmd": windows_cmd, "GptBridge.sh": linux_sh}
...
def build_release_tree(root: Path, output: Path, inputs: ReleaseInputs) -> dict[str, Any]:
    ...
    shutil.copy2(root / "LICENSE", licenses / "GptBridge-LICENSE.txt")
    ...
    version_manifest = {
        "schemaVersion": 1,
        "application": {"name": "GptBridgeScrcpy", "version": __version__},
...
def validate_release_tree(root: Path, *, require_offline_ready: bool = False) -> dict[str, Any]:
    ...
    errors: list[str] = []
    required = [
        "VERSION.json", "PACKAGE-MANIFEST.json", "README.txt", "gpt_bridge", "apps/web-client/dist/index.html",
        "compatibility/scrcpy-versions.json", "server/scrcpy-server.manifest.json",
        "licenses/GptBridge-LICENSE.txt", "licenses/scrcpy-LICENSE.txt", "licenses/THIRD_PARTY_NOTICES.txt",
    ]
