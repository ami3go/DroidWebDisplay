...
def test_running_app_endpoint_reports_verified_move_truthfully() -> None:
    root = Path(__file__).resolve().parents[2]
    source = (root / "droid_web_display" / "api" / "app.py").read_text(encoding="utf-8")
    start = source.index('@app.post("/api/v1/sessions/{session_id}/virtual-display/move-running-app")')
    end = source.find("\n    @app.", start + 1)
    ...
    assert '"status": "moved" if after else "launch-sent-unverified"' in route
    assert '"moved": after is not None' in route
    assert '"verified": after is not None' in route
    assert '"launchRequested": True' in route
    assert '"moved": True' not in route
